...
class URNAFlameGraphGenerator:
    ...
    # ANSI颜色代码
    COLORS = {
        'urma_write': '#FF6B6B',    # 红色
        'urma_read': '#4ECDC4',     # 青色
        'urma_send': '#45B7D1',     # 蓝色
        'urma_recv': '#96CEB4',      # 绿色
        'urma_poll_jfc': '#FFEAA7',  # 黄色
        'default': '#DDA0DD'         # 浅紫色
    }
        ...
        if flamegraph_path:
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"FlameGraph生成失败: {result.stderr.decode()}")
                    
            except Exception as e:
                print(f"执行FlameGraph失败: {e}")
        
        # 如果FlameGraph不可用，生成简单的HTML可视化
    ...
    def _generate_html_flamegraph(self, folded_data, output_file):
        ...
        for stack, duration in folded_data.items():
            func_name = stack.split()[0]
...
"""
        ...
        <div class="chart">
            <h2 style="margin-bottom: 20px; color: #4ECDC4;">时延分布</h2>
...
"""
        ...
        <div class="chart">
            <h2 style="margin-bottom: 20px; color: #4ECDC4;">详细统计</h2>
...
"""
    ...
    def _generate_json_report(self, output_file):
        ...
        # 按函数统计
        func_stats = defaultdict(lambda: {'count': 0, 'total_us': 0, 'max_us': 0, 'min_us': float('inf')})
        ...
        for func_name, stats in func_stats.items():
            ...
            if count > 0:
                report['summary'][func_name] = {
                    'count': count,
                    'total_latency_us': stats['total_us'],
                ...
                # 生成直方图分布
                buckets = [0] * 10  # 10个区间
                # 简化: 假设最大值为 max_us，平均分布
                max_us = stats['max_us']
                for event in self.events:
                    if event.get('func_name') == func_name:
                        duration = event.get('duration_us', 0)
                        bucket_idx = min(9, int((duration / max_us) * 10) if max_us > 0 else 0)
                        buckets[bucket_idx] += 1
                
                report['distribution'][func_name] = buckets
    ...
    def generate(self):
        ...
        print("URMA时延分析摘要")
        print("="*60)
        for func_name, stats in sorted(report['summary'].items(), 
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python3 %(prog)s -i output/urma_stacks_20240101_120000.txt
  python3 %(prog)s --input output/urma_stacks_20240101_120000.txt --output output/

输出文件:
  - SVG火焰图: 可使用FlameGraph渲染
  - HTML火焰图: 浏览器直接打开查看
  - JSON报告: 机器可读的分析数据
        """
    )
    ...
    parser.add_argument('-o', '--output', 
                       ...
                       help='输出目录 (默认: /root/URMATEST/output)')
